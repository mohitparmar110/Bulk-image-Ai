...
const App: React.FC = () => {
  ...
  const [settings, setSettings] = useState<GenerationSettings>({
    preset: LifestylePreset.SCANDINAVIAN,
    customPrompt: '',
    aspectRatio: '1:1'
  });
  ...
  const startProcessing = async () => {
    ...
    for (let i = 0; i < images.length; i++) {
      ...
      try {
        ...
        const resultUrl = await generateLifestyleImage(
          base64, 
          img.file.type, 
          finalPrompt,
  ...
  const downloadAll = () => {
    images.filter(img => img.status === 'completed' && img.resultUrl).forEach((img, index) => {
      const link = document.createElement('a');
      link.href = img.resultUrl!;
      link.download = `lifestyle-${img.file.name.split('.')[0]}.png`;
      ...
      <header className="bg-white/80 backdrop-blur-md border-b border-slate-200 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-slate-900 tracking-tight">Bulk Lifestyle AI</h1>
              <p className="text-xs font-medium text-slate-500 uppercase tracking-wider">Product Visualizer</p>
            </div>
          </div>
          ...
          <div className="lg:col-span-4 xl:col-span-3 space-y-8">
            <section className="bg-white p-7 rounded-3xl shadow-sm border border-slate-200">
              ...
              <div className="space-y-6">
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-3">Environment Preset</label>
                  <select 
                  ...
                  >
                    {Object.entries(LifestylePreset).map(([key, value]) => (
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-3">Custom Scene (Overrides Preset)</label>
                  <textarea 
                    placeholder="e.g. Floating in a zero-gravity space station with neon lights..."
            ...
            <div className="p-6 bg-indigo-50/50 rounded-3xl border border-indigo-100">
               ...
               <p className="text-xs text-indigo-700 leading-relaxed">
                 For best results, upload images where the product is centered and clear. Gemini will automatically match the horizon and lighting.
               </p>
            </div>
          </div>
          ...
          <div className="lg:col-span-8 xl:col-span-9">
            {images.length === 0 ? (
              <div 
                ...
                className="group relative border-4 border-dashed border-slate-200 bg-white rounded-[40px] p-20 text-center hover:border-indigo-400 hover:bg-indigo-50/50 transition-all cursor-pointer flex flex-col items-center justify-center min-h-[500px]"
              >
                <input 
                  type="file" 
                  ref={fileInputRef}
                  ...
                  className="hidden" 
                />
                <div className="bg-indigo-600 p-6 rounded-[2rem] text-white mb-8 shadow-2xl shadow-indigo-200 group-hover:scale-110 transition-transform">
                  <svg className="w-12 h-12" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M12 4v16m8-8H4" />
                  </svg>
                </div>
                <h3 className="text-3xl font-black text-slate-900 mb-4">Bulk Upload Products</h3>
                <p className="text-slate-500 max-w-md mx-auto text-lg leading-relaxed">
                  Drop your white-background product shots here. We'll generate contextual lifestyle environments in seconds.
                </p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-8">
                    ...
                    <div className="p-5 flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-3">
                        {img.status === 'completed' && img.resultUrl && (
                          <a 
                            href={img.resultUrl} 
                            download={`lifestyle-${img.file.name}`}
                ...
                {!isProcessing && images.length > 0 && (
                  <button 
                    onClick={() => fileInputRef.current?.click()}
                    className="aspect-square rounded-[2rem] border-4 border-dashed border-slate-100 flex flex-col items-center justify-center text-slate-300 hover:border-indigo-300 hover:text-indigo-400 hover:bg-indigo-50 transition-all group"
                  >
                    <input 
                      type="file" 
                      ref={fileInputRef}
                      ...
                      className="hidden" 
                    />
                    <div className="bg-slate-50 p-5 rounded-full mb-3 group-hover:bg-white group-hover:scale-110 transition-all">
                      <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M12 4v16m8-8H4" />
                      </svg>
                    </div>
                    <span className="text-xs font-bold uppercase tracking-widest">Add Images</span>
                  </button>
                )}
              </div>
            )}
